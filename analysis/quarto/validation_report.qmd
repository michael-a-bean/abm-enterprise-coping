...
See `docs/THRESHOLD_JUSTIFICATION.md` for rationale.
...
sensitivity_results |>
  ...
  gt::tab_header(
    ...
    subtitle = "Primary estimand (enterprise ~ price_exposure) across classification thresholds"
  ) |>
  gt::tab_footnote(
    footnote = "Robust conclusions require consistent negative sign across thresholds.",
    locations = gt::cells_column_labels(columns = Sign)
  )
```

```{r sensitivity-assessment}
#| results: hold
...
cat(sprintf("Sign stability: %s\n", if (stability$sign_stable) "STABLE (all negative)" else "UNSTABLE"))
cat(sprintf("Significance: %s\n", if (stability$majority_significant) "Majority significant (p < 0.05)" else "Variable significance"))
cat(sprintf("Coefficient range: [%.4f, %.4f]\n", stability$coefficient_range[1], stability$coefficient_range[2]))
cat(sprintf("Coefficient CV: %.2f%%\n", stability$coefficient_cv * 100))
cat(sprintf("\nOverall: %s\n", stability$interpretation))
```

```{r sensitivity-plot}
#| label: fig-sensitivity
#| fig-cap: "Coefficient Estimates Across Thresholds"

ggplot(sensitivity_results, aes(x = factor(threshold), y = beta_price_exposure)) +
  geom_point(size = 4, color = "steelblue") +
  geom_errorbar(
    aes(ymin = beta_price_exposure - 1.96 * se_price_exposure,
        ymax = beta_price_exposure + 1.96 * se_price_exposure),
    width = 0.2, color = "steelblue"
  ) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  scale_x_discrete(labels = c("0.33" = "33%", "0.5" = "50%", "0.67" = "67%")) +
  labs(
    title = "Price Exposure Coefficient by Classification Threshold",
    subtitle = "Error bars show 95% confidence intervals",
    x = "Stayer Threshold",
    y = expression(beta[price_exposure])
  ) +
...
cat(sprintf("  - Threshold Sensitivity: %s\n", if (stability$stable) "ROBUST" else "CHECK RESULTS"))
...
# Overall pass/fail (includes sensitivity check)
all_pass <- fe_results$pass && asset_results$pass && credit_results$pass && stability$stable
if (dist_results$mode == "validation") {
