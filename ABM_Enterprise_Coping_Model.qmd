...
The core hypothesis under investigation is that **negative cash-crop price shocks induce enterprise entry as coping behavior**, with heterogeneous responses by asset holdings and credit access.
...
1. **Asset Heterogeneity:** $\beta_2 < 0$ for interaction term (price_exposure × low_assets)
2. **Credit Heterogeneity:** $\beta_2 < 0$ for interaction term (price_exposure × no_credit)

These estimands are specified in `docs/VALIDATION_CONTRACT.md` and tested in the R validation pipeline.
...
The ABM incorporates some but not all characteristics typically associated with Complex Adaptive Systems (CAS):

**Implemented:**

- **Heterogeneity:** Agents differ in initial endowments and respond differently to identical shocks.
...
- **Emergence:** The current model does not exhibit emergent aggregate patterns not programmed into individual agents.
- **Agent learning/adaptation:** Agents do not update their decision rules based on experience.
- **Agent-agent interaction:** No social network effects or peer influences.

This is a "simple ABM" rather than a full CAS model. The CAS framing is partial and should not be overclaimed.
...
- **Population:** 500-1000 households per country (full sample varies)
...
First principal component of household durables, livestock, and land. Standardized to mean=0, SD=1.
...
- **Copers** show higher enterprise entry rates in shock waves (wave 2 for Tanzania)
...
Acceptance: p > 0.05 (cannot reject that distributions are identical).
...
- KS test assumes continuous distributions; application to binary outcomes is imperfect.
- Chi-squared test requires sufficient expected counts; small cells may violate assumptions.
- Neither test proves distributions are identical, only that we cannot reject similarity.

## Threshold Sensitivity Analysis

Core conclusions are tested across multiple stayer classification thresholds:

| Threshold | Stayer Definition | Tanzania Waves Required |
|-----------|-------------------|-------------------------|
| 33% | >33% enterprise waves | ≥2 of 4 waves |
| 50% | >50% enterprise waves | ≥3 of 4 waves |
| 67% | >67% enterprise waves | ≥3 of 4 waves |

**Acceptance:** Sign stability across all thresholds; majority of results significant at p < 0.05.
...
### Gemini 2.0 Flash (Sections 1-3 Methodology Review)

**Purpose:** Review Introduction, Research Question, and ABM/CAS Justification for methodological correctness.

**Key Feedback Incorporated:**

1. Changed "validated against" to "calibrated using" to avoid overclaiming
2. Added explicit justification for 50% stayer threshold
3. Documented what CAS features are and are not implemented
4. Added limitations on computational cost, parameter uncertainty, and oversimplification
5. Clarified that agents have continuous variables but discrete decision-making

### GPT-4o (Sections 8-10 Inference Review)

**Purpose:** Red-team reproducibility claims, inference validity, and policy interpretation.

**Key Feedback Incorporated:**

1. Replaced "BehaviorSpace-style robustness" with "Monte Carlo robustness checks"
2. Clarified p-value threshold interpretation (conventional, not definitive)
3. Added caveats about structural comparability for simulation-to-real comparison
4. Emphasized exploratory nature of findings
5. Strengthened external validity limitations statement
