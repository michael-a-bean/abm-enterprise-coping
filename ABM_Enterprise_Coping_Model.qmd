...
**Data Limitations:** Calibration relies on balanced panel data from LSMS-ISA surveys. To the extent that enterprise entry is associated with migration (household split-offs) not fully captured in panel tracking, the model may underrepresent high-mobility entrepreneurial trajectories. Panel attrition, particularly among young adults who may be more likely to start enterprises and migrate, is a potential source of survivor bias.
...
| Issue | Fix Applied |
|-------|-------------|
| A. External model consultation | Used Gemini Deep Research for methodology (Sections 1-3) and OpenAI o1 for statistics (Sections 8-10) |
...
5. **Structural Invariance (Lucas Critique Robustness):** Unlike reduced-form econometric models, the use of deep behavioral parameters (decision thresholds) allows for policy analysis that is potentially more robust to structural breaks. Regression coefficients (e.g., "price elasticity of entry") may change under new policy regimes as agents alter behavior; threshold-based rules that model the decision process itself may be more stable.
...
- **General equilibrium effects:** Prices and returns to enterprise are exogenous; household decisions do not affect aggregate price levels or local market saturation. This is a significant simplification: if many households simultaneously enter enterprise during a shock, crowding-out effects (reduced profits per enterprise) would occur in reality but are not captured here.
- **Market-level feedbacks:** No agent-to-agent interactions or market clearing mechanisms. The model does not account for local competition effects when multiple households enter similar enterprise types.
...
**Important Caveat:** This model is best characterized as an **agent-based microsimulation**. While it incorporates some elements associated with complex adaptive systems (CAS), such as heterogeneity and non-linearity, it is **not** a fully realized CAS model. It prioritizes the **cognitive realism** of threshold-based decision rules over the interactional complexity of full CAS models, allowing for clearer isolation of household-level coping drivers.
...
- **Non-linearity:** Threshold-based decision rules create discontinuous responses to price exposure. This captures "tipping point" dynamics where small changes in exposure can trigger discrete behavioral shifts.
- **Path dependence:** Enterprise persistence classification depends on history of participation.
- **Bounded rationality:** Agents follow heuristic decision rules (threshold-based) rather than optimizing utility functions, reflecting behavioral realism in data-scarce environments.
...
This is an agent-based microsimulation designed for transparent mechanism exploration rather than a full CAS model. Results should not be interpreted as capturing emergent system dynamics.
...
- **Non-equivalence:** Failing to reject the null hypothesis (p > 0.05) does **not** prove distributions are equivalent. It means the test lacked power to detect a difference, or the difference is small. Equivalence testing would require different methodology (e.g., TOST—Two One-Sided Tests—procedures).
- **KS limitations:** The KS test assumes continuous distributions; application to binary outcomes is imperfect and may have reduced power. For discrete outcomes, exact permutation tests may be more appropriate.
- **Chi-squared limitations:** Requires sufficient expected counts (typically ≥5 per cell); small cells may violate assumptions. When counts fall below this threshold, categories should be merged or exact tests used.
- **Multiple testing:** Running multiple distribution tests increases false positive risk. Consider applying Benjamini-Hochberg FDR correction or Bonferroni adjustment when interpreting multiple test results. This documentation does not currently implement formal multiple testing correction.
- **Power considerations:** The power of these tests depends on sample size and the magnitude of expected differences. With N=500 households per country, the KS test has moderate power to detect distributional differences. Small but meaningful differences may go undetected, and non-significant results should not be interpreted as evidence of equivalence.
- **Measurement error:** Endogeneity or measurement error in observed data may affect the validity of comparisons between simulated and observed distributions.
...
External model consultations were conducted to audit documentation for scientific validity and statistical correctness. Full call logs are available in `docs/EXTERNAL_MODEL_CALL_LOGS.md`.

### Gemini Deep Research (Sections 1-3 Methodology Review)

**Purpose:** Comprehensive methodology audit including CAS justification, overclaim detection, and ABM vs. microsimulation classification.

**Key Feedback Incorporated:**

1. Reframed "Simple ABM" terminology to "Agent-Based Microsimulation" for methodological precision
2. Added Lucas Critique justification to "Why ABM" section (structural invariance argument)
3. Added crowding-out/general equilibrium exclusion caveat with specific market saturation language
4. Added panel attrition bias caveat for LSMS-ISA data limitations
5. Softened causal language: "simulates... as coping mechanism" → "investigates... coping dynamics"
6. Added bounded rationality as explicit CAS feature (heuristic vs. optimization)
7. Recommended "exploratory validation" framing for causal claims

**Methodology Audit Matrix (from Gemini Deep Research):**

| Feature | Verdict | Action Taken |
|---------|---------|--------------|
| "Coping Mechanism" framing | Caution | Clarified as exploratory investigation |
| "Partial CAS" designation | Refine | Changed to "agent-based microsimulation" |
| Heterogeneity justification | Weak standalone | Added bounded rationality argument |
| Missing ABM justification | Critical | Added Lucas Critique point |
| Generalizability caveats | Incomplete | Added attrition and crowding-out notes |

### OpenAI o1 (Sections 8-10 Statistical Review)

**Purpose:** Red-team statistical methodology, p-value interpretations, sensitivity analysis, and policy inference claims.

**Key Feedback Incorporated:**

1. Confirmed p > 0.05 interpretation language is appropriately conservative
2. Added multiple testing correction note (Benjamini-Hochberg, Bonferroni)
3. Added power considerations note for distribution tests
4. Added measurement error caveat
5. Added TOST (Two One-Sided Tests) as explicit alternative for equivalence testing
6. Confirmed policy inference framing is appropriately cautious
7. Recommended documenting outlier handling for chi-square tests
...
|-------|----------|---------|--------|
| Gemini Deep Research | 1-3 | Methodology/CAS/overclaim audit | Success |
| OpenAI o1 | 8-10 | Statistical/inference review | Success |

**Full logs:** See `docs/EXTERNAL_MODEL_CALL_LOGS.md` for complete prompts and responses.

**Consensus recommendations incorporated:**

1. Reframed terminology from "Simple ABM" to "Agent-Based Microsimulation"
2. Added Lucas Critique (structural invariance) justification for ABM approach
3. Added crowding-out and panel attrition caveats for data limitations
4. Added multiple testing correction and power considerations notes
5. Strengthened statistical test interpretation with explicit non-equivalence caveats
6. Added bounded rationality as explicit CAS-adjacent feature
