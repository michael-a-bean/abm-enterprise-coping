...
::: {.callout-warning title="Synthetic Data Notice"}
The batch robustness analysis, parameter sweeps, and behavior search in this section use **synthetic uncalibrated** data generated by `generate_synthetic_households()`. This is NOT calibrated to LSMS distributions. Results should be interpreted as exploratory sensitivity analysis, not validated empirical findings.

To generate calibration-based results, run: `abm calibrate --country tanzania` then use `SyntheticPanelGenerator`.
:::
...
#| fig-cap: "Distribution of enterprise rates across 10 replicate runs with different random seeds. Shaded region shows mean +/- 1 SD. The narrow distribution indicates stable outcomes across stochastic realizations. **Data source:** Synthetic uncalibrated (toy mode, seeds 1-10, N=100)."
...
```{r}
#| label: fig-heatmap
#| fig-cap: "Policy sensitivity heatmap showing enterprise rate as a function of price threshold and asset threshold. Data from 72 simulation runs (6x6 grid, 2 seeds each). **Data source:** Synthetic uncalibrated (N=100, seeds 42-43). See DATA_CONTRACT.md for provenance."

# Load real sweep data from parameter sweep
sweep_path <- here::here("outputs", "sweeps", "sweep_agg_latest.parquet")
...
#| tbl-cap: "Top 5 parameter configurations from random search (40 candidates, 2 seeds each). Objective is MSE between simulated and target enterprise rates. **Data source:** Synthetic uncalibrated (N=100). **Target source:** Hardcoded LSMS stylized facts."

search_path <- here::here("outputs", "search", "candidates_latest.parquet")
...
| @fig-robustness | `outputs/batch/seed_*` | Synthetic uncalibrated | 100 | 1-10 | Toy mode |
| @tbl-robustness-metrics | `outputs/batch/seed_*` | Synthetic uncalibrated | 100 | 1-10 | |
| @fig-heatmap | `outputs/sweeps/sweep_agg_latest.parquet` | Synthetic uncalibrated | 100 | 42-43 | 6x6 grid |
| @tbl-search-results | `outputs/search/candidates_latest.parquet` | Synthetic uncalibrated | 100 | 42-43 | 40 candidates |
| @fig-search-objective | `outputs/search/candidates_latest.parquet` | Synthetic uncalibrated | 100 | 42-43 | |

: Data provenance for all figures and tables. {#tbl-data-provenance}

## Data Integrity Audit

See `docs/DATA_AUDIT.md` for the complete audit report. Key findings:

1. **FLAG 1 (CRITICAL):** Sweep/search use uncalibrated synthetic data, not LSMS-calibrated
2. **FLAG 2 (HIGH):** LLM policy results not yet generated; report describes design only
3. **FLAG 3 (MEDIUM):** Inconsistent N values across runs (500 vs 100)
4. **FLAG 4 (MEDIUM):** Batch runs use toy mode, not baseline configuration
